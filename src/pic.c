...
static hopper_err_t encode_comp3(const hopper_field_t *field, int32_t value, uint8_t *dst, uint32_t len) {
  ...
  for (int di = digits - 1; di >= 0; di--, nib--) {
...
static hopper_err_t decode_comp3(const hopper_field_t *field, const uint8_t *src, uint32_t len, int32_t *out) {
  ...
  uint8_t sign_nibble = (uint8_t)(src[len - 1] & 0x0F);
  int sign = 1;
  if (sign_nibble == 0xD) {
    sign = -1;
  } else if (sign_nibble == 0xC || sign_nibble == 0xF) {
    sign = 1;
  } else {
    return HOPPER_E_PIC_INVALID;
  }
  if (!field->pic.is_signed && sign < 0) {
    return HOPPER_E_PIC_INVALID;
  }

  uint16_t digits = field->pic.digits;
  uint64_t acc = 0;
  for (uint16_t di = 0; di < digits; di++) {
    uint32_t nib_index = di;
    uint32_t byte_index = nib_index / 2;
    uint8_t nibble = (nib_index % 2 == 0) ? (uint8_t)(src[byte_index] >> 4) : (uint8_t)(src[byte_index] & 0x0F);
    if (nibble > 9) {
      return HOPPER_E_PIC_INVALID;
    }
    acc = acc * 10u + nibble;
